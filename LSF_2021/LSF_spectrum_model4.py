...
if __name__ == "__main__":
    ...
    test_model = lsp_spectrum_model4()
    test_model.summary()
    ...
    filepath = "lsf_spectrum_model4/lsf_spectrum_model4-{epoch:02d}-{val_loss:.8f}.h5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1,
                                 save_best_only=True, mode='auto')  # only save improved accuracy model

    callbacks_list = [checkpoint]
    history = test_model.fit(x=[lsf_train, f0_train, uv_train], y=y_train, batch_size=64, epochs=2000,
