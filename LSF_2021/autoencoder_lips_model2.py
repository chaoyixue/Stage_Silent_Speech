...
def autoencoder_lips():
    ...
    # encoding
    conv1 = Conv2D(filters=16, kernel_size=(3, 3), activation='relu', padding='same')(input_img)
    pooling1 = MaxPooling2D(pool_size=(2, 2))(conv1)
    conv2 = Conv2D(filters=8, kernel_size=(3, 3), activation='relu', padding='same')(pooling1)
    pooling2 = MaxPooling2D(pool_size=(2, 2))(conv2)
    conv3 = Conv2D(filters=8, kernel_size=(3, 3), activation='relu', padding='same')(pooling2)
    encoded = MaxPooling2D(pool_size=(2, 2))(conv3)

    # decoding
    conv4 = Conv2D(filters=8, kernel_size=(3, 3), activation='relu', padding='same')(encoded)
    upsample1 = UpSampling2D(size=(2, 2))(conv4)
    conv5 = Conv2D(filters=8, kernel_size=(3, 3), activation='relu', padding='same')(upsample1)
    upsample2 = UpSampling2D(size=(2, 2))(conv5)
    conv6 = Conv2D(filters=16, kernel_size=(3, 3), activation='relu', padding='same')(upsample2)
    upsample3 = UpSampling2D(size=(2, 2))(conv6)
    decoded = Conv2D(filters=1, kernel_size=(3, 3), activation='sigmoid', padding='same')(upsample3)
...
if __name__ == "__main__":
    ...
    filepath = "autoencoder_lips_model2/autoencoder_lips_model2-{epoch:02d}-{val_loss:.8f}.h5"
