...
def five_images_to_LSF_model2():
    ...
    tongues_flat_layer = TimeDistributed(Flatten())(tongues_pooling2)
    cc = concatenate([lips_flat_layer, tongues_flat_layer])
    lstm1 = LSTM(512, return_sequences=False)(cc)
    lstm_dr1 = Dropout(0.2)(lstm1)
    fc = Dense(13, activation="linear")(lstm_dr1)
    mymodel = Model([input_lips, input_tongues], fc)
    return mymodel
...
if __name__ == "__main__":
    ...
    lsf_original = np.load("lsp_cut_all.npy")
    # spectrum of ch1-ch6
    training_labels = lsf_original[0:nb_training_images]
    validation_labels = lsf_original[-15949:-2]
    ...
    # model
    model11 = five_images_to_LSF_model1()
    model11.summary()
    ...
    filepath = "data_coupe_five_images_to_lsf_model1/data_coupe_five_images_to_lsf_model1-{epoch:02d}-{val_loss:.8f}.h5"
    ...
    history = model11.fit_generator(training_generator,
                                    ...
                                    epochs=100,
                                    validation_data=([validation_lips, validation_tongues], validation_labels),
