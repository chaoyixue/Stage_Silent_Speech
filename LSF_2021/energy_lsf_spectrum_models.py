...
if __name__ == "__main__":
    # this version used data cut already

    # load data
    X_lsf = np.load("lsp_cut_all.npy")
    X_f0 = np.load("f0_cut_all.npy")
    X_uv = np.load("uv_cut_all.npy")
    spectrum = np.load("spectrogrammes_all_chapitres_coupe.npy")
    energy = np.load("energy_cut_all.npy")
    ...
    test_model = energy_lsf_spectrum_model2()
    test_model.summary()
    ...
    filepath = "energy_lsf_spectrum_data_coupe_model2/" \
               "energy_lsf_spectrum_data_coupe_model2-{epoch:02d}-{val_loss:.8f}.h5"
