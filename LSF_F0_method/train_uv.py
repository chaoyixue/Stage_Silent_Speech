from LSF_F0_method.ssi_cnn import *
import wave
import numpy as np
...
from LSF_F0_method.utils.lsf_utils import *
import pandas as pd
...
def keras_train_pred_uv(name='lipstongue2uv_', egg=False,
                        ...
                        IS16=True, multi_task=True, optimizer='adam', lr=0.0001,
                        AE=True, epochs=30, order=1, stepbystep=False):
    EXP_NAME = name
    try:
    ...
    print(len(train_tongue), len(train_ys[0]))
    if multi_task:
        ...
        if stepbystep:
            ...
            aucscore = roc_auc_score(test_y, predict[:, 1])
            accuracyscore = accuracy_score(test_y.astype(int), (predict[:, 1] > 0.5).astype(int))

            print('auc', aucscore, 'accuracy', accuracyscore)
        ...
        for i in range(order):
            ...
            model_checkpoint = tf.keras.callbacks.ModelCheckpoint(model_path, monitor='val_loss', save_best_only=True,
                                                                  save_weights_only=True)
            print(len(test_tongue), len(test_ys[i]))
            hist = model.fit([train_tongue, train_lips], train_ys[i],
                             validation_split=0.05,
                             batch_size=512, epochs=epochs, shuffle=True,
                             callbacks=[earlystop, model_checkpoint])
            bst_epoch_inter = np.nanargmax(hist.history['val_loss'])
