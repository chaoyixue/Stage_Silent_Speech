...
def musicdata_wavereader(normalized=True):
    ...
    for song in songs:
        ...
        print("_ed wave %s: nchannels %d, sampwidth %d, framerate %d, nframes %d" % (
...
def simple_audio2lsf(audio_data, order):
    lpcfilter = lpc(audio_data, order)
...
def vocoder(lsf, activation, fs=16000, hamming=True, frame_length=1 / 60, overlap_percent=50, frame_pixel=None):
    ...
    print(signal_length)
    downsample_audio_data = np.hstack(
        [np.array([0] * int(exact_frame_length / 2)), activation, np.array([0] * int(exact_frame_length / 2))])
    ...
    for i in tqdm(range(int(signal_length / exact_frame_length))):
        ...
        if hamming:
            frame = downsample_audio_data[start_index:stop_index] * spsig.windows.hamming(stop_index - start_index)
        else:
