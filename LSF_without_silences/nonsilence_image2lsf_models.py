...
def nonsilence_image2lsf_model1():
...
def nonsilence_image2lsf_model2():
    ...
    lip_conv6 = Conv2D(filters=64, kernel_size=(3, 3), activation='relu', padding='same')(lip_conv5)
    lip_pooling3 = MaxPooling2D(pool_size=(2, 2))(lip_conv6)
    lip_conv7 = Conv2D(filters=128, kernel_size=(3, 3), activation='relu', padding='same')(lip_pooling3)
    lip_conv8 = Conv2D(filters=128, kernel_size=(3, 3), activation='relu', padding='same')(lip_conv7)
    ...
    tongue_conv4 = Conv2D(filters=32, kernel_size=(3, 3), activation='relu', padding='same')(tongue_conv3)
    tongue_pooling2 = MaxPooling2D(pool_size=(2, 2))(tongue_conv4)
    tongue_conv5 = Conv2D(filters=64, kernel_size=(3, 3), activation='relu', padding='same')(tongue_pooling2)
    tongue_conv6 = Conv2D(filters=64, kernel_size=(3, 3), activation='relu', padding='same')(tongue_conv5)
...
def nonsilence_image2lsf_model3():
    input_lip = Input(shape=(64, 64, 1))
    # encoding
    lip_conv1 = Conv2D(filters=16, kernel_size=(5, 5), activation='relu', padding='same')(input_lip)
    lip_conv2 = Conv2D(filters=16, kernel_size=(5, 5), activation='relu', padding='same')(lip_conv1)
    ...
    flat_tongue = Flatten()(tongue_pooling2)
    cc = concatenate([flat_lip, flat_tongue])
    lsf_fc0 = Dense(512, activation='relu', name='lsf_fc0')(cc)
    lsf_dr0 = Dropout(0.5)(lsf_fc0)
    lsf_fc1 = Dense(256, activation='relu', name='lsf_fc1')(lsf_dr0)
    lsf_dr1 = Dropout(0)(lsf_fc1)
    lsf_fc2 = Dense(13, activation='linear', name='lsf_fc2')(lsf_dr1)
    new_model = Model([input_lip, input_tongue], lsf_fc2, name='transfer_autoencoder_lsf')
    new_model.summary()
...
if __name__ == "__main__":
    X_lip = np.load("lip_nonsilence_all_chapiter.npy")
    X_tongue = np.load("tongue_nonsilence_all_chapiter.npy")
    lsf_original = np.load("lsf_nonsilence_all_chapiter.npy")
    ...
    test_model = nonsilence_image2lsf_model3()
    ...
    filepath = "nonsilence_image2lsf_model3_dr5000/nonsilence_image2lsf_model3_dr5000-{epoch:02d}-{val_loss:.8f}.h5"
