...
from ssi4_input_data_classif import *
import torch
...
def match_image_label1(image_data):  #3D
    l=image_data.shape[0]
    image_match=[]
    for i in range(l-5):
        image_con = []
        image_con.extend(image_data[i:i+6])
        image_match.append(image_con)
    image_match = np.array(image_match)

    return image_match

def match_image_label2(image_data): #3D
...
def match_image_label(image_data): #2D
    ...
    image_match=[]
    for i in range(l-5):
        image_con=np.concatenate((image_data[i:i+6]),axis=-1) #(batch_size, 64, 64, 6)
        image_match.append(image_con)
    image_match = np.array(image_match)
...
def SSIDatasets():
    ...
    train_lips, test_lips, train_tongue, test_tongue, train_label, test_label = load_dataset_classif()
    # train_lips = train_lips[:30005,:,:,:]
    # train_tongue = train_tongue[:30005,:,:,:]
    # train_label = train_label[:30000,:]
    # test_lips = test_lips[6000:,:,:,:]
    # test_tongue = test_tongue[6000:,:,:,:]
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(6, 16, kernel_size=3, bias=True),#（2*64*64) 若在卷积后加bn，最好bias=False
            nn.LeakyReLU(),
        ...
        self.dense1 = nn.Sequential(
            nn.Linear(30752, 4096), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.LeakyReLU(),   
            nn.Dropout(0.2))
        self.dense2 = nn.Sequential(
            nn.Linear(4096, 512), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.LeakyReLU(),   
            nn.Dropout(0.2))
...
# loss_func = nn.BCEWithLogitsLoss() #sigmoid+bceloss
    ...
    for epoch in range(NUM_EPOCH):
        ...
        with torch.no_grad():
            ...
            for step,(test_lips, test_tongue, test_label) in enumerate(eval_loader):
                ...
                correct+=(output==test_label).sum().item()
            eval_losses.append(eval_loss/len(eval_datasets))
            # print('=====> Epoch:',epoch+1, ' | Average epoch eval loss: %.4f ' % (eval_loss/len(eval_datasets)))
            print('=====> Epoch:',epoch+1, ' | Average epoch eval loss: %.4f ' % (eval_loss/len(eval_datasets)), ' | Average epoch eval accuracy: %.4f ' %(100*correct/total))
...
def test_model():
    ...
    for step,(test_lips, test_tongue, test_label) in enumerate(test_loader):
        ...
        if step==0:
            ...
            prediction=torch.cat((prediction,mask),0) #按行竖着接
