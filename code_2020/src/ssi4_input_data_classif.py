...
def _get_spectrograms(y, sr=44100, n_fft=2048, win_length=1024, hop_length=512):
	...
	# mag_trans=[]
	# for i in range(l):
	# 	mag_trans.append(mag[i].max())
	# mag_trans = np.array(mag_trans)
	# print(mag_trans.shape)
	# return mag_trans
	return mag
...
def _get_melspectrograms(y,sr,n_fft,n_mels,max_db,ref_db):
	...
	# 	# print(mel[i].shape)
	# 	if i==0:
	# 		m = (mel[i].max()+mel[i+1].max())/2
	# 	elif i==l-1:
	# 		m= (mel[i-1].max()+mel[i].max())/2
	# 	else:
	# 		m = (mel[i-1].max()+mel[i].max()+mel[i+1].max())/3
		m=[]
		if mel[i].max()>=-40:
			m.append(1)
		else:
			m.append(0)
		mel_trans.append(m)
		# mel_trans.append(mel[i].max())
	mel_trans = np.array(mel_trans)
	print(mel_trans.shape)
	return mel_trans
	# return mel
...
def load_dataset_classif(dir_path="../data/database/"):
	if os.path.exists(dir_path):
		arr = []
		for num in enumerate(['train_lips', 'train_tongue', 'test_lips', 'test_tongue', 'cla_train_label', 'cla_test_label']):
		...
		test_lips.extend(lips_data[61331:])
		test_tongue.extend(tongue_data[61331:])
...
def _generate_audio_mel_Lable(dir_path = './ssi/data/Songs_Audio_matched/out01234.wav'):
	...
	melspec = librosa.feature.melspectrogram(_y,sr=sr,n_fft=5120, hop_length=735, win_length=4410, power=1.2, n_mels=64) #(64, 68147)
	...
	print(':::::')
	train_label, test_label = [], []
	train_label.extend(magnitude[:61326])
	...
	np.save("/Users/yawen/Documents/ssi/ssi/data/database/64new_train_label.npy", train_label)
	np.save("/Users/yawen/Documents/ssi/ssi/data/database/64new_test_label.npy", test_label)

	return train_label, test_label

def _generate_audio_mel_Lable_yuanshikeyong(dir_path = './ssi/data/Songs_Audio_matched/out01234.wav'):
	sr=44100
	preempha=0.95
	n_fft=5120
	win_length = 4410
	...
	magnitude.extend(mel[3:-3])
	# magnitude.extend(_mag[3:-3])
	# print(_mag.shape)
	print(len(magnitude)) #50087310/hop+1
	...
	np.save("./ssi/data/database/cla_train_label.npy", train_label)
	np.save("./ssi/data/database/cla_test_label.npy", test_label)
...
def _generate_audio_mel_Lable2(dir_path = './ssi/data/Songs_Audio_matched/'):
	...
	np.save("/Users/yawen/Documents/ssi/ssi/data/database/train_label.npy", train_label)
	np.save("/Users/yawen/Documents/ssi/ssi/data/database/test_label.npy", test_label)
...
if __name__ == '__main__':
	...
	train_label, test_label = _generate_audio_mel_Lable_yuanshikeyong('./ssi/data/Songs_Audio_matched/out01234.wav')
