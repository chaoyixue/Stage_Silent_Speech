...
def SSIDatasets():
    ...
    #change dimension match: (x,2,64,64,1) --> (x,1,2,64,64)
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(1,16,kernel_size=(2,3,3),stride=1,padding=1,bias=True),
        ...
        self.conv2 = nn.Sequential(
            nn.Conv3d(16,32,kernel_size=(2,3,3),stride=1,padding=1,bias=True),
            nn.BatchNorm3d(32),
            nn.LeakyReLU(),
            nn.MaxPool3d(2))        # (32*16*16)
            # nn.Dropout(0.2))
        ...
        self.dense1 = nn.Sequential(
            nn.Linear(16384, 2048), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.LeakyReLU())
            # nn.Dropout(0.2))
