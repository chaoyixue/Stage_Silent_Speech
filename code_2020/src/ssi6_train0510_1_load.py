...
def SSIDatasets():
    ...
    train_lips = train_lips[30001:,:,:,:]
    train_tongue = train_tongue[30001:,:,:,:]
    train_label = train_label[30001:,:]
    ...
    #change dimension match: (x,6,64,64,1) --> (x,1,6,64,64) input:(batch_size,in_channal,in_depth,in_height,in_width)
    train_lips = train_lips.permute(0,4,1,2,3).cuda()  
    test_lips = test_lips.permute(0,4,1,2,3).cuda()  
    train_tongue = train_tongue.permute(0,4,1,2,3).cuda()  
    test_tongue = test_tongue.permute(0,4,1,2,3).cuda()  
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(1,10,kernel_size=(6,3,3),stride=1,padding=1,bias=True), #conv3d:(in_channal,out_channals,kernel...)
            # nn.Conv2d(2, 16, kernel_size=3, bias=True),#（2*64*64) 若在卷积后加bn，最好bias=False
            nn.LeakyReLU(),
            nn.MaxPool3d(2),       #（16*32*32）X
            # nn.Dropout(0.2),
            nn.BatchNorm3d(10))
        ...
        self.conv4 = nn.Sequential(
            nn.Conv3d(1,10,kernel_size=(6,3,3),stride=1,padding=1,bias=True),
            # nn.Conv2d(2, 16, kernel_size=3, bias=True),
            nn.LeakyReLU(),
            nn.MaxPool3d(2),
            # nn.Dropout(0.2),
            nn.BatchNorm3d(10))
        ...
        self.dense1 = nn.Sequential(
            nn.Linear(20480, 4096), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.LeakyReLU(),   
            nn.Dropout(0.2))
