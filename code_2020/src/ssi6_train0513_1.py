...
def match_image_label(image_data):
    ...
    for i in range(l-11):
        image_con = []
        image_con.extend(image_data[i:i+12])
...
def SSIDatasets():
    ...
    train_lips = train_lips.permute(0,4,1,2,3)
    test_lips = test_lips.permute(0,4,1,2,3)
    train_tongue = train_tongue.permute(0,4,1,2,3) 
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(1,16,kernel_size=(12,3,3),stride=1,padding=1,bias=True), #conv3d:(in_channal,out_channals,kernel...)
        ...
        self.conv4 = nn.Sequential(
            nn.Conv3d(1,16,kernel_size=(12,3,3),stride=1,padding=1,bias=True),
...
def main():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        with torch.no_grad(): #前向传播中不记录梯度，节省内存
