...
def match_image_label(image_data): #2D
    ...
    image_match=[]
    for i in range(l-6):
        image_con=np.concatenate((image_data[i:i+7]),axis=-1) #(batch_size, 64, 64, 6)
        image_match.append(image_con)
    image_match = np.array(image_match)
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(7, 16, kernel_size=3, padding=1, bias=True),#（2*64*64) 若在卷积后加bn，最好bias=False
            nn.ReLU(),
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(7, 16, kernel_size=3, padding=1, bias=True),
            nn.ReLU(),
            nn.Conv2d(16, 16, kernel_size=3, padding=1, bias=True),
            nn.ReLU(),
            nn.MaxPool2d(2),    #（16*32*32）
