...
def match_image_label(image_data): #2D
    ...
    image_match=[]
    for i in range(l-8):
        image_con=np.concatenate((image_data[i:i+9]),axis=-1) #(batch_size, 64, 64, 6)
        image_match.append(image_con)
    image_match = np.array(image_match)
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(9, 16, kernel_size=3, padding=1, bias=True),#（2*64*64) 若在卷积后加bn，最好bias=False
            nn.ReLU(),
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(9, 16, kernel_size=3, padding=1, bias=True),
            nn.ReLU(),
            nn.Conv2d(16, 16, kernel_size=3, padding=1, bias=True),
            nn.ReLU(),
            nn.MaxPool2d(2),    #（16*32*32）
        ...
        self.dense1 = nn.Sequential(
            nn.Linear(16384, 1024), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.ReLU(),
            nn.Dropout(0.2))
        self.dense2 = nn.Sequential(
            nn.Linear(1024, 256), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.ReLU())
            # nn.Dropout(0.2))
