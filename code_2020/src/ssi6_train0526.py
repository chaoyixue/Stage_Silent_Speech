...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(13, 16, kernel_size=3, padding=1, bias=True),#（2*64*64) 若在卷积后加bn，最好bias=False
            nn.ReLU(),
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(13, 16, kernel_size=3, padding=1, bias=True),
            nn.ReLU(),
            nn.Conv2d(16, 16, kernel_size=3, padding=1, bias=True),
            nn.ReLU(),
            nn.MaxPool2d(2),    #（16*32*32）
...
if __name__ == "__main__":
    ...
    # np.save(root+"train_losses.npy", train_losses.detach().numpy())
    # np.save(root+"eval_losses.npy", eval_losses.detach().numpy())
