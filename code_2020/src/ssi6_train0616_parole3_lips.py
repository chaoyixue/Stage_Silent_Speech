...
def main():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        train_loss=0.0
        #step_loss=0.0
        for step, (train_lips, _, train_label) in enumerate(train_loader):
            train_lips, train_label = Variable(train_lips).cuda(), Variable(train_label).cuda()
            optimizer.zero_grad()     #梯度值初始化为0
            output = model(train_lips)
        ...
        with torch.no_grad():
            ...
            eval_loss=0.0
            for step,(test_lips, _, test_label) in enumerate(eval_loader):
                test_lips, test_label = Variable(test_lips).cuda(), Variable(test_label).cuda()
                output = model(test_lips)
...
def test_model():
    ...
    for step,(test_lips, _, test_label) in enumerate(test_loader):
        test_lips, test_label = Variable(test_lips).cuda(), Variable(test_label).cuda()
        output = model(test_lips)
        loss = loss_func(output,test_label)
