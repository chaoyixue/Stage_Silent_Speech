...
def SSIDatasets():
    ...
    train_lips = train_lips.permute(0,1,4,2,3) 
    test_lips = test_lips.permute(0,1,4,2,3)
    train_tongue = train_tongue.permute(0,1,4,2,3)
    test_tongue = test_tongue.permute(0,1,4,2,3)
    print(train_lips.shape,train_tongue.shape,train_label.shape)
...
class CNN(nn.Module):
    def cnn3d(self,in_c,out_c,k_s=3,stri=1,padd=1):
        conv = nn.Sequential(
            ...
            nn.ReLU(),
            # nn.Conv3d(out_c, out_c, kernel_size=k_s, stride=stri, padding=padd, bias=True),
            # nn.ReLU(),
            nn.MaxPool3d(2),
    ...
    def __init__(self):
        ...
        self.conv1 = self.cnn3d(1,16)
        self.conv2 = self.cnn3d(16,32)
        self.conv3 = self.cnn3d(32,64)

        self.conv4 = self.cnn3d(1,16)
        self.conv5 = self.cnn3d(16,32)
        self.conv6 = self.cnn3d(32,64)

        self.dense1 = nn.Sequential(
            nn.Linear(20480, 1024), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.ReLU(),
            nn.Dropout(DROPOUT))
