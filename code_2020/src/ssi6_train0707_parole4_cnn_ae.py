...
class AutoEncoder(nn.Module):
    def __init__(self):
        ...
        self.encoder1 = nn.Sequential(
            nn.Conv2d(i, 16, kernel_size=5, padding=2, bias=True),#（2*64*64) 若在卷积后加bn，最好bias=False
            nn.ReLU(),
        ...
        self.decoder1 = nn.Sequential(
            nn.ConvTranspose2d(32, 32, kernel_size=3, padding=1, bias=True),
            nn.ReLU(),
            nn.ConvTranspose2d(32, 16, kernel_size=3, padding=1, bias=True),
            nn.ReLU(),
            # nn.Dropout(DROPOUT),
            nn.Upsample(scale_factor=2,mode='nearest'),    #或者bilinear
            nn.BatchNorm2d(16))
        self.decoder2 = nn.Sequential(
            nn.ConvTranspose2d(16, 16, kernel_size=3, padding=1,bias=False),
            nn.ReLU(),
            nn.ConvTranspose2d(16, i, kernel_size=3, padding=1,bias=False),
            nn.ReLU(),
            # nn.Dropout(DROPOUT),
            nn.Upsample(scale_factor=2,mode='nearest'),    #或者bilinear
            nn.BatchNorm2d(i))
        ...
        self.dense1 = nn.Sequential(
            nn.Linear(32*16*16*2, 1024), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.ReLU(),
            nn.Dropout(DROPOUT))
    ...
    def forward(self, lips, tongue):
        ...
        out2 = self.encoder3(tongue)              
        out2 = self.encoder4(out2)                
        out_tongue = self.decoder4(out2)
        out_tongue = self.decoder5(out_tongue)
        out_tongue = self.decoder6(out_tongue)
