...
class CNN_LSTM(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(i, 16, kernel_size=3, padding=1, bias=True),#（batch_size,time_step,(channel,)64,64) 若在卷积后加bn，最好bias=False
            nn.ReLU(),
        ...
        #     nn.Linear(4096, 1024), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
        #     nn.ReLU())
        #     nn.Dropout(0.5))
        ...
        self.lstm1 = nn.LSTM(input_size=64, hidden_size=512, num_layers=2, batch_first=True, bidirectional=True)
        self.lstm2 = nn.LSTM(input_size=64, hidden_size=512, num_layers=2, batch_first=True, bidirectional=True)
