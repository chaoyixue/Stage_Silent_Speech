...
class CNN_LSTM(nn.Module):
    def cnn2d(self,in_c, out_c,k_s=3,padd=1):
        conv = nn.Sequential(
            ...
            nn.BatchNorm2d(out_c,momentum=MOMENTUM))
            # nn.Dropout(DROPOUT))
        return conv
    ...
    def __init__(self):
        ...
        self.lstm1 = nn.LSTM(input_size=64,hidden_size=512,num_layers=2,bias=True,batch_first=True,dropout=DROPOUT)
        self.lstm2 = nn.LSTM(input_size=64,hidden_size=512,num_layers=2,bias=True,batch_first=True,dropout=DROPOUT)
    ...
    def forward(self, lips, tongue):
        ...
        out1 = out1.view(out1.size(0),64,-1)  #(batch_size,timestep,-1)
        
        out2 = self.conv4(tongue)              
        out2 = self.conv5(out2)                
        out2 = self.conv6(out2)    
        # out2 = self.conv9(out2)
        out2 = out2.view(out2.size(0),64,-1) 

        out_lstm1, (h_n1,h_c1) = self.lstm1(out1,None)  #hidden states, h_n 是分线, h_c 是主线,none为0
        out_lstm2, (h_n2,h_c2) = self.lstm2(out2,None)  #hidden states, h_n 是分线, h_c 是主线,none为0
        out_lstm = torch.cat((out_lstm1[:, -1, :], out_lstm2[:, -1, :]),dim=1)
        out = self.dense1(out_lstm)
        # out = self.linear(out_lstm[:, -1, :]) #h_n的值，最后一个时间点的lstm的输出

        # out_cnn = torch.cat((out1, out2),dim=2)
        # out_lstm, (h_n,h_c) = self.lstm(out_cnn,None)  #hidden states, h_n 是分线, h_c 是主线,none为0
        # out = self.dense1(out_rnn[:, -1, :])
        # # out = self.linear(out_lstm[:, -1, :]) #h_n的值，最后一个时间点的lstm的输出
