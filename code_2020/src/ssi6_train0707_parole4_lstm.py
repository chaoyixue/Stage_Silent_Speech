...
class LSTM(nn.Module):
    def __init__(self):
        ...
        self.lstm1 = nn.LSTM(input_size=64, hidden_size=512, num_layers=2, batch_first=True, bias=True,bidirectional=True)
        self.lstm2 = nn.LSTM(input_size=64, hidden_size=512, num_layers=2, batch_first=True, bias=True,bidirectional=True)
    ...
    def forward(self, lips, tongue):
        ...
        out1_lstm, (h_n1,h_c1) = self.lstm1(out1,None)  #hidden states, h_n 是分线, h_c 是主线,none为0
                     
        out2 = tongue.view(tongue.size(0),i*tongue.size(2),-1) 
        out2_lstm, (h_n2,h_c2) = self.lstm2(out2,None)  #hidden states, h_n 是分线, h_c 是主线,none为0

        out_lstm = torch.cat((out1_lstm[:, -1, :], out2_lstm[:, -1, :]),dim=1)
        out_lstm = self.dense1(out_lstm)
        out = self.dense2(out_lstm)
