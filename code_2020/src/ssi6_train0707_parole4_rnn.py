...
def SSIDatasets():
    ...
    test_lips = test_lips[:8461,:,:,:]
    test_tongue = test_tongue[:8461,:,:,:]
    test_label = test_label[:8460,:]
...
class RNN(nn.Module):
    def __init__(self):
        ...
        self.rnn1 = nn.RNN(input_size=64, hidden_size=512, num_layers=2, batch_first=True, bias=True)
        self.rnn2 = nn.RNN(input_size=64, hidden_size=512, num_layers=2, batch_first=True, bias=True)
    ...
    def forward(self, lips, tongue,h1,h2):
        ...
        out1_rnn, h1 = self.rnn1(out1,h1)  #hidden states, h_n 是分线, h_c 是主线,none为0,input of shape (seq_len, batch, input_size)
                     
        out2 = tongue.view(tongue.size(0),i*lips.size(2),-1) 
        out2_rnn, h2 = self.rnn2(out2,h2)  #hidden states, h_n 是分线, h_c 是主线,none为0,output of shape (seq_len, batch, num_directions * hidden_size)

        out_rnn = torch.cat((out1_rnn[:, -1, :], out2_rnn[:, -1, :]),dim=1)
        out = self.dense1(out_rnn)
