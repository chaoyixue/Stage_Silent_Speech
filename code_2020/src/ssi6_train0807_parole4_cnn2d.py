...
def SSIDatasets():
    ...
    train_lips, test_lips, train_tongue, test_tongue, _, _ = load_dataset()
    train_label=np.load(root+'train_label_ae.npy')
    test_label=np.load(root+'test_label_ae.npy')
    if i==1 or i==2:
        pass
    else:
...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        #     nn.Linear(16384, 8096), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
        #     nn.ReLU(),
        #     nn.Dropout(DROPOUT))
...
def main():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for step, (train_lips, train_tongue, train_label) in enumerate(train_loader):
            ...
            loss = loss_func(output,train_label.view(train_label.size(0),1,-1).squeeze(1))
            loss.backward()           #反向传播                  
            optimizer.step()          #更新参数
        ...
        with torch.no_grad():
            ...
            for step,(test_lips, test_tongue, test_label) in enumerate(eval_loader):
                ...
                loss = loss_func(output,test_label.view(test_label.size(0),1,-1).squeeze(1))
...
def test_model():
    ...
    for step,(test_lips, test_tongue, test_label) in enumerate(test_loader):
        test_lips, test_tongue, test_label = Variable(test_lips).cuda(), Variable(test_tongue).cuda(), Variable(test_label).cuda()
        output = model(test_lips, test_tongue)
        loss = loss_func(output,test_label.view(train_label.size(0),1,-1).squeeze(1))
