...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = self.cnn2d(i,128)
        self.conv2 = self.cnn2d(128,64)
        self.conv3 = self.cnn2d(64,32)
        self.conv7 = self.cnn2d(32,16) #[32 x 512]
        # self.conv8 = self.cnn2d(128,256) # [32 x 64]

        self.conv4 = self.cnn2d(i,128)
        self.conv5 = self.cnn2d(128,64)
        self.conv6 = self.cnn2d(64,32)
        self.conv9 = self.cnn2d(32,16)
        ...
        self.dense2 = nn.Sequential(
            nn.Linear(512, 736), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.ReLU())
            # nn.LeakyReLU())
            # nn.Sigmoid())# 旧，jyr
            # nn.Softmax()#分类
...
def main():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for step, (train_lips, train_tongue, train_label) in enumerate(train_loader):
            ...
            loss2 = loss_func2(output[:,350:],train_label[:,350:])
        ...
        with torch.no_grad():
            ...
            for step,(test_lips, test_tongue, test_label) in enumerate(eval_loader):
                ...
                loss2 = loss_func2(output[:,350:],test_label[:,350:])
                loss = loss1+loss2
