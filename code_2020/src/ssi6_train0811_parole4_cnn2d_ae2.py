...
def SSIDatasets():
    ...
    train_label=np.load(root+'train_predict_me_ae.npy') #(76210, 736) (76211, 64, 64, 1) (76211, 64, 64, 1)
    if i==1 or i==2:
        pass
    else:
    ...
    test_label=train_label[70000:,:]
    train_lips=train_lips[:70000,:,:,:]
    train_tongue=train_tongue[:70000,:,:,:]
    train_label=train_label[:70000-i+1,:]
...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = self.cnn2d(i,64,k_s=5)
        # self.conv2 = self.cnn2d(16，32)
        # self.conv3 = self.cnn2d(32,64)
        self.conv7 = self.cnn2d(64,128,k_s=5)
        self.conv8 = self.cnn2d(128,256)

        self.conv4 = self.cnn2d(i,64,k_s=5)
        # self.conv5 = self.cnn2d(i,32)
        # self.conv6 = self.cnn2d(32,64)
        self.conv9 = self.cnn2d(64,128,k_s=5)
        self.conv10 = self.cnn2d(128,256)

        self.dense1 = nn.Sequential(
            nn.Linear(18432, 1024), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.ReLU(),
            nn.Dropout(DROPOUT))
    ...
    def forward(self, lips, tongue):
        ...
        out = self.dense1(out)
        out = self.dense2(out) #torch.Size([batchsize, 368])
...
def main():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for step, (train_lips, train_tongue, train_label) in enumerate(train_loader):
            ...
            loss1 = loss_func1(output[:,:350],train_label[:,:350])
            loss2 = loss_func1(output[:,350:],train_label[:,350:])
        ...
        with torch.no_grad():
            ...
            for step,(test_lips, test_tongue, test_label) in enumerate(eval_loader):
                ...
                loss1 = loss_func1(output[:,:350],test_label[:,:350])
                loss2 = loss_func1(output[:,350:],test_label[:,350:])
                loss=10*loss1+0.1*loss2
