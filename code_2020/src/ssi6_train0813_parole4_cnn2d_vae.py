...
def SSIDatasets():
    ...
    train_label=np.load(root+'train_label_ae.npy') #vae([76204, 1, 30])
...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.dense2 = nn.Sequential(
            nn.Linear(512, 30), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.ReLU())
            # nn.LeakyReLU())
            # nn.Sigmoid())# 旧，jyr
            # nn.Softmax()#分类
...
def main():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for step, (train_lips, train_tongue, train_label) in enumerate(train_loader):
            ...
            # train_label=train_label.view(train_label.size(0),1,-1).squeeze(1)
        ...
        with torch.no_grad():
            ...
            for step,(test_lips, test_tongue, test_label) in enumerate(eval_loader):
                ...
                loss = loss_func(output,test_label) #(32,30)
