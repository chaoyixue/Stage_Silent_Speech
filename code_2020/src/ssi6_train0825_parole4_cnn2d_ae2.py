...
def SSIDatasets():
    ...
    train_f0=f0[:70000-i+1,:]
    train_label=np.concatenate((train_label,train_f0),axis=1)
...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.dense2 = nn.Sequential(
            nn.Linear(1024, 737), # 卷积核3*3*16， 64-3+1=62， 输出62*62*16
            nn.ReLU())
            # nn.LeakyReLU())
            # nn.Sigmoid())# 旧，jyr
            # nn.Softmax()#分类
...
def main():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for step, (train_lips, train_tongue, train_label) in enumerate(train_loader):
            ...
            loss1 = loss_func1(output[:,:735],train_label[:,:735])
            loss2 = loss_func1(output[:,735:],train_label[:,735:])
            loss=loss1+loss2
            loss.backward()           #反向传播                  
            optimizer.step()          #更新参数
        ...
        with torch.no_grad():
            ...
            for step,(test_lips, test_tongue, test_label) in enumerate(eval_loader):
                ...
                loss1 = loss_func1(output[:,:735],test_label[:,:735])
                loss2 = loss_func1(output[:,735:],test_label[:,735:])
                loss=loss1+loss2
