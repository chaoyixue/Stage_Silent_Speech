...
def main():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        with torch.no_grad():
            ...
            _, lsf, _, f0, _, _, _, uv, _, _ = load_dataset()
            ...
            print(prediction_f0[:5,:])
            print(prediction_lsf[:5,:])
            print(prediction_uv[:5,:])
            ...
            roc_auc=roc_auc_score(prediction_uv.cpu().detach().numpy(), uv)
            print(prediction_f0[:5,:],prediction_f0.shape,f0.shape)
            # f0 = np.clip((f0 - min_f0) / (max_f02 - min_f0), 1e-8, 1)   #nor
            prediction_f0 = (np.clip(prediction_f0.cpu().detach().numpy(),1e-8,1)*(max_f02-min_f0))+min_f0  #denor
            print(prediction_f0[:5,:])
            ...
            print('=====> Epoch:',epoch+1, ' | Pearson Corr. Coef. (f0): %.8f , %.8f ' % (r,pro))
            print('=====> Epoch:',epoch+1, ' | ROC AUC(v/uv): %.8f ' % roc_auc)
            print('=====> Epoch:',epoch+1, ' | Average epoch v/uv accuracy: %.8f ' %(100*correct/total))
