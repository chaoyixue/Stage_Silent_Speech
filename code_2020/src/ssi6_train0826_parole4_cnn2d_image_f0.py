...
def SSIDatasets():
    ...
    # print('2: ',train_lsf.shape, test_lsf.shape, train_f0.shape, test_f0.shape, train_lips.shape, test_lips.shape, train_uv.shape, test_uv.shape,train_tongue.shape, test_tongue.shape )
...
def main():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for step, (_, train_f0, _, train_lips, train_tongue) in enumerate(train_loader):
            train_f0, train_lips, train_tongue =  Variable(train_f0).cuda(),Variable(train_lips).cuda(), Variable(train_tongue).cuda()
            optimizer.zero_grad()     #梯度值初始化为0
            out_f0 = model(train_lips, train_tongue)
            loss = loss_func2(out_f0,train_f0)
            loss.backward()           #反向传播                  
            optimizer.step()          #更新参数
        ...
        with torch.no_grad():
            ...
            _, _, _, f0, _, _, _, _, _, _ = load_dataset()
            for step,(_, test_f0, _, test_lips, test_tongue) in enumerate(eval_loader):
                test_f0, test_lips, test_tongue = Variable(test_f0).cuda(), Variable(test_lips).cuda(), Variable(test_tongue).cuda()
                out_f0 = model(test_lips, test_tongue)
                loss = loss_func2(out_f0,test_f0)
            ...
            # print(prediction_f0[:5,:],prediction_f0.shape,f0.shape)
            # prediction_f0 = (np.clip(prediction_f0.cpu().detach().numpy(),0,1)*(max_f02-min_f0))+min_f0  #denor
            # r,pro = pearsonr(prediction_f0.flatten(), f0.flatten()) #第一个值为pearson相关系数，第二个为p-value
            print(prediction_f0[:5,:],prediction_f0.cpu().detach().numpy().flatten().mean(),f0.flatten().mean())
            # f0 = np.clip((f0 - min_f0) / (max_f02 - min_f0), 1e-8, 1)   #nor
            # pref0_s = StandardScaler().fit_transform(prediction_f0.cpu().detach().numpy())
            # f0_s=StandardScaler().fit_transform(f0)
            pref0_s=preprocessing.scale(prediction_f0.cpu().detach().numpy())
            f0_s=preprocessing.scale(f0)
            print(pref0_s.shape,f0_s.shape)
            print(pref0_s[:,:5],pref0_s.mean(),f0_s.mean())
            r,pro = pearsonr(pref0_s.flatten(), f0_s.flatten()) #第一个值为pearson相关系数，第二个为p-value
            # r,pro = pearsonr(prediction_f0.cpu().detach().numpy().flatten(), f0.flatten()) #第一个值为pearson相关系数，第二个为p-value
            # _,_,r1,pro1,_=linregress(prediction_f0.cpu().detach().numpy().flatten()[:2], f0.flatten()[:2])
            print('=====> Epoch:',epoch+1, ' | Average epoch eval loss: %.8f ' % (eval_loss/len(eval_datasets)))
            print('=====> Epoch:',epoch+1, ' | Pearson Corr. Coef. (r_value and p_value): %.8f , %.8f ' % (r,pro))
            # print('=====> Epoch:',epoch+1, ' | Pearson Corr. Coef.2 (r_value and p_value): %.8f , %.8f ' % (r1,pro1))
...
if __name__ == "__main__":
    ...
    plt.ylabel('loss')
    plt.grid(True)
    plt.savefig(root+"epoch_loss_f0.png")
    ...
    np.save(root+"train_losses_f0.npy", np.array(train_losses))
    np.save(root+"eval_losses_f0.npy", np.array(eval_losses))
