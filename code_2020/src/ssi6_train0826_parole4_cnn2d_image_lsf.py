...
def measure(ytrue, ypred,order=12):
    predict = ypred
    test_lsf = ytrue
    # predict = [val.flatten() for val in predict]
    # predict_lsf = np.array(predict)
    test_lsf = test_lsf.transpose()
...
def spectral_distortion(lsf_true, lsf_pred, N, n0, n1):
    ...
    for frameid in range(len(lsf_true)):
        ...
        _, freqResponse_true = signal.freqz(b=1, a=lpc_true, worN=512)
        _, freqResponse_pred = signal.freqz(b=1, a=lpc_pred, worN=512)
...
def main():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for step, (train_lsf, _, _, train_lips, train_tongue) in enumerate(train_loader):
            train_lsf, train_lips, train_tongue = Variable(train_lsf).cuda(), Variable(train_lips).cuda(), Variable(train_tongue).cuda()
            optimizer.zero_grad()     #梯度值初始化为0
            out_lsf = model(train_lips, train_tongue)
            loss = loss_func1(out_lsf.unsqueeze(1),train_lsf)
            loss.backward()           #反向传播                  
            optimizer.step()          #更新参数
        ...
        with torch.no_grad():
            ...
            _, lsf, _, _, _, _, _, _, _, _ = load_dataset()
            for step,(test_lsf, _, _, test_lips, test_tongue) in enumerate(eval_loader):
                test_lsf, test_lips, test_tongue = Variable(test_lsf).cuda(), Variable(test_lips).cuda(), Variable(test_tongue).cuda()
                out_lsf= model(test_lips, test_tongue)
                loss = loss_func1(out_lsf.unsqueeze(1),test_lsf)
                eval_loss += float(loss.item()*test_lips.size(0))
                if step==0:
                    prediction_lsf = out_lsf.unsqueeze(1)
                else:
                    prediction_lsf=torch.cat((prediction_lsf,out_lsf.unsqueeze(1)),0)
            eval_losses.append(eval_loss/len(eval_datasets))
            sd, is16sd,predict_lsf = measure(lsf[:,:-2,:].squeeze(2),prediction_lsf.permute(2,0,1).cpu().detach().numpy().squeeze(2))
            print('=====> Epoch:',epoch+1, ' | Average epoch eval loss: %.8f ' % (eval_loss/len(eval_datasets)))
            print('=====> Epoch:',epoch+1, ' | is16sd (LSF): %.8f ' % is16sd)
...
if __name__ == "__main__":
    ...
    plt.ylabel('loss')
    plt.grid(True)
    plt.savefig(root+"epoch_loss_lsf.png")
    ...
    np.save(root+"train_losses_lsf.npy", np.array(train_losses))
    np.save(root+"eval_losses_lsf.npy", np.array(eval_losses))
