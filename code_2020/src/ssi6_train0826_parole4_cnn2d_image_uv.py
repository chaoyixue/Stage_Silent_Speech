...
def main():
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for step, (_,_,train_uv, train_lips, train_tongue) in enumerate(train_loader):
            train_uv, train_lips, train_tongue = Variable(train_uv).cuda(),Variable(train_lips).cuda(), Variable(train_tongue).cuda()
            optimizer.zero_grad()     #梯度值初始化为0
            out_uv = model(train_lips, train_tongue)
            loss = loss_func3(out_uv,train_uv)
            loss.backward()           #反向传播                  
            optimizer.step()          #更新参数
        ...
        with torch.no_grad():
            ...
            _, _, _, _, _, _, _, uv, _, _ = load_dataset()
            for step,(_, _, test_uv, test_lips, test_tongue) in enumerate(eval_loader):
                test_uv, test_lips, test_tongue = Variable(test_uv).cuda(),Variable(test_lips).cuda(), Variable(test_tongue).cuda()
                out_uv = model(test_lips, test_tongue)
                loss = loss_func3(out_uv,test_uv)
...
if __name__ == "__main__":
    ...
    plt.ylabel('loss')
    plt.grid(True)
    plt.savefig(root+"epoch_loss_uv.png")
    ...
    np.save(root+"train_losses_uv.npy", np.array(train_losses))
    np.save(root+"eval_losses_uv.npy", np.array(eval_losses))
