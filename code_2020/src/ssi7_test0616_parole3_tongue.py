...
from ssi6_train0616_parole3_tongue import CNN, match_image_label
...
def test_model(test_datasets, test_loader):
    ...
    for step,(_, test_tongue, test_label) in enumerate(test_loader):
        ...
        test_mae += float(mae*test_tongue.size(0))     
        if step==0:
            prediction=output
        else:
