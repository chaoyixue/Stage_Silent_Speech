...
from ssi6_train0707_parole4_cnn2d import CNN, match_image_label
...
def TestDatasets1():
    ...
    test_lips = test_lips[14800:15200+i-1,:,:,:]
    test_tongue = test_tongue[14800:15200+i-1,:,:,:]
    test_label = test_label[14800:15200,:]
...
def TestDatasets2():
    ...
    print('[INFO] set datasets')
    # _, test_lips, _, test_tongue, _, test_label = load_dataset()
    test_lips = np.load('../data/new_database/ssnv_train_lips.npy')
    test_tongue = np.load('../data/new_database/ssnv_train_tongue.npy')
    ...
    #两通道
    test_datasets2 = TensorDataset(test_lips, test_tongue)
...
def TestDatasets3():
    ...
    print('[INFO] set datasets')
    _, test_lips, _, test_tongue, _, test_label = load_dataset()
    test_lips = np.load('../data/new_database/ssnv_test_lips.npy')
    test_tongue = np.load('../data/new_database/ssnv_test_tongue.npy')
    ...
    #两通道
    test_datasets3 = TensorDataset(test_lips, test_tongue)
...
def test_model1(test_datasets, test_loader):
    print('[INFO] start testing, output predict')
    # model.eval() #不启用batchnormalization和dropout
    test_loss=0.0
    mae, test_mae=0.0, 0.0
...
if __name__ == "__main__":
    ...
    np.save(root+"test_predict_me1_ssv.npy", spec1)

    test_datasets2, test_loader2 = TestDatasets2()
    print('[INFO] begin test')
    prediction2 = test_model1(test_datasets2, test_loader2)
    print('[INFO] save test output')
    spec2 = prediction2.cpu().detach().numpy()
    np.save(root+"test_predict_me2_ssv.npy", spec2)

    test_datasets3, test_loader3 = TestDatasets3()
    print('[INFO] begin test')
    prediction3 = test_model1(test_datasets3, test_loader3)
    print('[INFO] save test output')
    spec3 = prediction3.cpu().detach().numpy()
    np.save(root+"test_predict_me3_ssv.npy", spec3)
