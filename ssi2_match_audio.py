...
def audio_matching(src, des, head, slope, out_frames, inchannels=1):
    ...
    print(len(data))
    sf.write(des, data, default_fs, subtype='FLOAT')
    
    return True 


try:
    os.mkdir('./ssi/data/Songs_Audio_matched')
except:
    pass
song_path = [
...
for i in range(len(song_path)):
    audio_matching('./ssi/data/audio/' + song_path[i], './ssi/data/Songs_Audio_matched/' + song_new_path[i], head[i],slope[i], out_frames[i]) 
