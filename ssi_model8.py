...
if __name__ == "__main__":
    # load data
    X_lips = np.load("lips_recurrent_5images_all_chapitres.npy")
    X_tongues = np.load("tongues_recurrent_5images_all_chapitres.npy")
    Y = np.load("spectrum_recurrent_all.npy")
    ...
    filepath = "../ssi_model8/ssi_model8-{epoch:02d}-{val_loss:.8f}.h5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1,
                                 save_best_only=True, mode='auto')  # only save improved accuracy model

    callbacks_list = [checkpoint]
    history = test_model.fit(x=[lips_x_train, tongues_x_train], y=y_train, batch_size=32, epochs=200,
